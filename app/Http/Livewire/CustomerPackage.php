...
class CustomerPackage extends Component
{
    ...
    public $ss = ['state'=> 0, 'custom'=> 0];
    public $subscription = '';

    public $mixed_metro_areas = [];
    public $mixed_metro_areas_name = [];
    ...
    public function fetch_banks($page)
    {
        ...
        $this->total_bank_count = $query->count();
    ...
    public function subscription_changed($value)
    {
        ...
        if($this->ss['state'] == 0 && $this->ss['custom'] == 0){
            $this->subscription = '';
        }elseif($this->ss['state'] == 1 && $this->ss['custom'] == 1){
            $this->subscription = 'msa';
        }elseif($this->ss['state'] == 1 && $this->ss['custom'] == 0){
            $this->subscription = 'state';
        }elseif($this->ss['state'] == 0 && $this->ss['custom'] == 1){
    ...
    public function mixed_select_metro_area($id)
    {
        if (!in_array($id, $this->mixed_metro_areas)) {
            array_push($this->mixed_metro_areas, $id);
            array_push($this->mixed_metro_areas_name, Bank::where('cbsa_code',$id)->pluck('cbsa_name')->first());
        // } else {
        //     if (!in_array($id, $this->bank_city_filter)) {
        //         array_push($this->bank_city_filter, $id);
        //         array_push($this->bank_city_filter_name, Bank::where('cbsa_code',$id)->pluck('cbsa_name')->first());
        ...
        if($this->subscription != ""){
            if ($this->subscription == 'custom') {
                ...
                $contract = Contract::create([
                    ...
                    'contract_end' => date('Y-m-d', strtotime(date('Y-m-d') . ' + 1 year + 1 Month')),
                ...
                $bank->display_reports = $this->subscription;
                $bank->save();
                foreach ($this->mixed_metro_areas as $key => $city) {
                    BankSelectedCity::create([
                        'bank_id' => $this->bank->id,
                        'city_id' => $city,
                    ]);
    ...
    public function delete_mixed_select_metro_area($item){
        $bank = Bank::where('cbsa_name',$this->mixed_metro_areas_name[$item])->first();
        unset($this->mixed_metro_areas[array_search($bank->cbsa_code,$this->mixed_metro_areas)]);
        unset($this->mixed_metro_areas_name[$item]);
        $this->page = 1;
        $this->all_banks = null;
    }
    ...
    public function calulate_current_amount(){
        if($this->subscription == ''){
            ...
            $standard = Packages::where('package_type', 'state')->first();
            $standard_rate = $standard->price*count($this->mixed_metro_areas);

            $charges = Packages::where('package_type', 'custom')->first();
            if(count($this->custom_banks) <= $charges->number_of_units){
                $custom_rate = $charges->price;
            }else{
                $custom_rate = $charges->price + ($charges->additional_price*(count($this->custom_banks)-$charges->number_of_units));
            }
            return $custom_rate+$standard_rate;
        }
    }
