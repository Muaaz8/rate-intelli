...
class AddBankRates extends Component
{
    ...
    public function upload_xlsx()
    {
        ActivityLog::create([
            ...
            'activity' => auth()->user()->name." Uploaded APY Rates.",
        ]);
                ...
                if($bank!=null){
                    ...
                    foreach ($head as $key => $hd) {
                        ...
                        && $hd!='Date (mm/dd/YYYY)' && $hd!=null){
                            ...
                            if($rt!=null){
                                $check = BankPrices::where('bank_id',$bank->id)->where('rate_type_id',$rt->id)->where('created_at','!=',$date)->orderby('id','DESC')->first();
                                if($check==null){
                                    if($dt[$hd] != null){
                                        if($dt[$hd] != "0"){
                                    ...
                                    if($dt[$hd] != null){
                                        if(abs($dt[$hd]-$check->current_rate) >= 1){
                                            $p_user = DB::table('bank_prices')->insertGetID([
                                                'bank_id' => $bank->id,
                                                'rate_type_id' => $rt->id,
                                                'rate' => $check->rate,
