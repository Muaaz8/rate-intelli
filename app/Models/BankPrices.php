...
class BankPrices extends Model
{
    ...
    //     return $rate_types;
    // }
    public static function SeperateReports($type, $code, $selectedBankType,$bank_list)
    {
        $userBank = auth()->user();
        $bankQuery = Bank::query();
            ...
            case 'all':
                if($bank_list == ""){
                    if ($filter->display_reports == 'state') {
                        ...
                        $cities = BankSelectedCity::where('bank_id', $filter->id)->pluck('city_id')->toArray();
                        $standard_banks_id = $bankQuery->where('cbsa_code', $cities);
        ...
        if($bank_list == ""){
            if($filter->display_reports == "custom"){
                ...
                $standard_banks_id = $standard_banks_id->pluck('id')->toArray();
            ...
            $banks = $bankQuery->whereIn('id',$bank_list)->pluck('id')->toArray();
        }
        ...
        $rateTypes['show_banks'] = Bank::join('bank_prices','bank_prices.bank_id','banks.id')
    ...
    public static function get_min_max_func($type,$state,$code,$selected_bank_type)
    {
        ...
        if($selected_bank_type == ""){
            if($type == 'state'){
                ...
                $cities = BankSelectedCity::where('bank_id',$filter->id)->pluck('city_id')->toArray();
                $standardselected_banks = Bank::whereIn('cbsa_code',$cities)->pluck('id')->toArray();
                $customselected_banks = CustomPackageBanks::where('bank_id',$filter->id)->pluck('customer_selected_bank_id')->toArray();
                $selected_banks = array_merge($standardselected_banks,$customselected_banks);
            }else{
                if($filter->display_reports == 'custom'){
                    $selected_banks = CustomPackageBanks::where('bank_id',$filter->id)->pluck('customer_selected_bank_id')->toArray();
                }elseif($filter->display_reports == 'state'){
            ...
            if($type == 'state'){
                if($filter->display_reports == 'state'){
                    if($code!= ""){
                        $selected_banks = Bank::where('cbsa_code',$code)->whereIn('bank_type_id',$selected_bank_type)->pluck('id')->toArray();
                    }else{
                ...
                if($filter->display_reports == 'custom'){
                    ...
                    $selected_banks = array_merge($standard_banks_id,$custom_banks_id);
                }
            }
        }
        foreach ($rate_types as $re_key => $rt) {
            ...
            $rt->c_max = BankPrices::whereIn('created_at', function ($query) use ($id,$selected_banks) {
                ...
                ->where('rate_type_id', $id)->where('is_checked',1);
    ...
    public static function summary_report($id,$selected_bank_type,$msa_code,$banks){
        ...
        //     }else{
        //         $banks = Bank::where('cbsa_code',$msa_code)->whereIn('bank_type_id',$selected_bank_type)->pluck('id')->toArray();
        //     }
        ...
        //         ->pluck('customer_selected_bank_id')->toArray();
        //     $standard_banks_id = Bank::where('cbsa_code', $cities)->whereIn('banks.bank_type_id',$selected_bank_type)->pluck('id')->toArray();
        //     $banks = array_merge($standard_banks_id,$custom_banks_id);
    ...
    public static function historic_report($id,$date,$banks_id){
        $data = BankPrices::select('bank_prices.*', 'banks.name as bank_name')
        ->join('banks', 'bank_prices.bank_id', 'banks.id')
        ->where('bank_prices.created_at','like','%'.$date.'%' )
        // function ($query) use ($id,$banks) {
        //     $query->selectRaw('MAX(created_at)')
        //         ->from('bank_prices')
        //         ->where('rate_type_id', $id)
        //         ->where('is_checked', 1)
